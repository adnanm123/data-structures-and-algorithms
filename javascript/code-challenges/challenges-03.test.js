...
const containsAnd = (arr) => {
  let filteredArr = arr.filter(value => value.includes('and'));
  return filteredArr;
};
...
const oddValues = (arr) => {
  let numbersArray = arr.filter(value => value % 2);
  return numbersArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let filteredArr = arr.filter(value => !forbiddenValues.includes(value));
  return filteredArr;
};
