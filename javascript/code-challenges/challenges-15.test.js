...
const screenForNames = (arr) => {
  let regex = /^(Mr\.|Dr\.|Ms\.|Mrs\.)\s?[a-z|A-Z]+$/;
  let newArr = arr.filter(name => regex.test(name));
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].name === 'Luke Skywalker') {
      lukeMass = parseInt(arr[i].mass);
      break;
    }
  }
  const names = [];
  for (let i = 0; i < arr.length; i++) {
    if (parseInt(arr[i].mass) > lukeMass) {
...
const isSecure = (url) => {
  if (url.startsWith('https://')) {
    // Check if there is a single '/' character after "https:"
    const protocolIndex = url.indexOf('https:') + 'https:'.length;
    const nextChar = url.charAt(protocolIndex);
    return nextChar === '/';
  }
  return false;
};
