...
const count = (target, input) => {
  return input.reduce((totalCount, array) => {
    const occurrences = array.filter(item => item === target).length;
...
const totalSum = (input) => {
  return input.reduce((total, array) => {
    return total + array.reduce((sum, value) => sum + value, 0);
  }, 0);
};
...
let findShortest = (data) => {
  const charactersWithHeight = data.map(character => ({ name: character.name, height: parseInt(character.height) || 0 }));
  const shortestCharacter = charactersWithHeight.reduce((shortest, character) => {
    return character.height < shortest.height ? character : shortest;
  }, charactersWithHeight[0]);
